...
class ApiService {
  ...
  /**
   * Créer une demande d'abonnement (POST /terrains/demandes-abonnement/)
   * Note: plan_id et terrain_id sont les clés attendues par le serializer backend
   */
  createDemandeAbonnement(payload: {
    terrain_id?: number;
    plan_id?: number;
  ...
  confirmerDisponibiliteDemande(demandeId: number, disponibilite: boolean, notes?: string) {
    const url = `${ENDPOINTS.demandesAbonnement}${demandeId}/confirmer_disponibilite/`;
    return this.requestNormalized<DemandeAbonnementDTO>(url, {
      method: 'POST',
      headers: this.headers(),
      body: JSON.stringify({ disponibilite_confirmee: disponibilite, notes_manager: notes }),
    });
  }

  changerStatutDemande(demandeId: number, statut: string, notes?: string) {
  ...
  annulerDemandeAbonnement(demandeId: number) {
    const url = `${ENDPOINTS.demandesAbonnement}${demandeId}/annuler/`;
    return this.requestNormalized<DemandeAbonnementDTO>(url, {
      method: 'POST',
      headers: this.headers(),
    });
  ...
  getMySubscriptions() {
    const url = `${ENDPOINTS.abonnements}`;
    return this.requestNormalized<SubscriptionResponseDTO[]>(url, {
      method: 'GET',
      headers: this.headers(),
    });
  ...
  deleteAdminTerrain(terrainId: number) {
    const url = `${API_ROOT}/admin/terrains/${terrainId}/`;
    return this.requestNormalized(url, {
      method: 'DELETE',
      headers: this.headers(),
