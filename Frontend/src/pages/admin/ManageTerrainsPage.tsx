...
const ManageTerrainsPageSimple: React.FC = () => {
  ...
  const deleteTerrain = async (terrain: Terrain) => {
    ...
    try {
      const { meta } = await apiService.deleteAdminTerrain(terrain.id);
      if (meta?.success === false) {
        throw new Error(meta.message || 'Erreur lors de la suppression');
      }
      toast.success(`Terrain "${terrain.nom}" supprimé.`);
      setTerrains((prev) => prev.filter((t) => t.id !== terrain.id));
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Erreur lors de la suppression du terrain.';
      toast.error(message);
    } finally {
      setDeletingId(null);
    }
