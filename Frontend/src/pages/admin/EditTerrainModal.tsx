...
const EditTerrainModal: React.FC<EditTerrainModalProps> = ({ terrain, onClose, onSuccess }) => {
  ...
  const [form, setForm] = useState({
    ...
    taux_commission: terrain.gestionnaire?.taux_commission_defaut?.toString() || '10',
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Si un gestionnaire est lié, mettre à jour son taux de commission par défaut
      if (terrain.gestionnaire?.id && form.taux_commission) {
        try {
          await apiService.updateManagerCommission(
            terrain.gestionnaire.id,
            parseFloat(form.taux_commission),
            'Mis à jour via édition terrain'
          );
          console.log('Taux de commission du gestionnaire mis à jour');
        } catch (err) {
          console.warn('Échec mise à jour commission gestionnaire:', err);
          // Ne pas bloquer le succès de la sauvegarde du terrain
        }
      }
          ...
          <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Pourcentage prélevé sur chaque réservation de ce terrain (modifie le taux par défaut du gestionnaire)
              </p>
            </div>
