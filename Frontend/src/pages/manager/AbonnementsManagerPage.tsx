...
const AbonnementsManagerPage: React.FC = () => {
  ...
  const handleDisponibilite = async (demandeId: number, confirm: boolean) => {
    ...
    try {
      const { meta } = await apiService.confirmerDisponibiliteDemande(demandeId, confirm);
      toast.success((meta?.message as string) || 'Disponibilité mise à jour');
      await fetchDemandes();
    } catch (error) {
  ...
  const handleStatut = async (demandeId: number, nouveauStatut: string) => {
    setProcessingId(demandeId);
    try {
      const { meta } = await apiService.changerStatutDemande(demandeId, nouveauStatut);
      toast.success((meta?.message as string) || 'Statut mis à jour');
      await fetchDemandes();
    } catch (error) {
