...
try {
    ...
    foreach ($terrains as $terrain) {
        ...
        // Vérifier si le prix manque
        if (empty($terrain->prix_par_heure) || $terrain->prix_par_heure == 0) {
            $needs_update = true;
        ...
        // Vérifier si la capacité manque
        if (empty($terrain->capacite) || $terrain->capacite == 0) {
            $needs_update = true;
    ...
    foreach ($terrains_final as $terrain) {
        if (!empty($terrain->prix_par_heure) && $terrain->prix_par_heure > 0 && 
            !empty($terrain->capacite) && $terrain->capacite > 0) {
            $complete_count++;
        }
    }
