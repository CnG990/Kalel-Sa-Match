...
class _OtpVerificationScreenState extends State<OtpVerificationScreen> {
  ...
  Future<void> _verifyOTP() async {
    ...
    try {
      final response = await _apiService.verifyOTP(widget.phoneNumber, otpCode);
      ...
      if (response['success'] == true) {
        ...
        if (isNewUser || widget.isRegistration) {
          // Nouvel utilisateur - aller à la création du PIN puis aux informations personnelles
          ...
          // Utilisateur existant - aller à la connexion avec PIN
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (context) => PinLoginScreen(phoneNumber: widget.phoneNumber),
            ),
          );
        }
      } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response['message'] ?? 'Code OTP incorrect'),
            backgroundColor: Colors.red,
          ),
        );
        // Effacer les champs
