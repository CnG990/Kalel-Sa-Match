...
class ProfileView(APIView):
    ...
    def post(self, request):
        ...
        if not current_password or not new_password:
            return Response({'data': None, 'meta': {'success': False, 'message': 'Mot de passe actuel et nouveau mot de passe requis'}}, status=400)

        if not request.user.check_password(current_password):
            return Response({'data': None, 'meta': {'success': False, 'message': 'Mot de passe actuel incorrect'}}, status=400)

        if len(new_password) < 8:
            return Response({'data': None, 'meta': {'success': False, 'message': 'Le mot de passe doit contenir au moins 8 caractères'}}, status=400)

        request.user.set_password(new_password)
        request.user.save()
        return Response({'data': {'id': request.user.id}, 'meta': {'success': True, 'message': 'Mot de passe modifié avec succès'}})
